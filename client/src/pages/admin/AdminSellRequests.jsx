...
const AdminSellRequests = () => {
    ...
    const fetchRequests = async () => {
        try {
            const res = await axios.get('/api/admin/requests/sell-device');
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`/api/admin/requests/sell-device/${editingItem.id}`, {
                status: editingItem.status,
                final_offer_price: editingItem.final_offer_price,
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                <h1 style={{ fontSize: '2rem', fontWeight: 'bold' }}>Sell Device Requests</h1>
                    ...
                    <table className="table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border-light)' }}>
                                ...
                                <th style={{ padding: '15px' }}>Device Details</th>
                                <th style={{ padding: '15px' }}>Condition</th>
                                <th style={{ padding: '15px' }}>Est. Price</th>
                                <th style={{ padding: '15px' }}>Final Offer</th>
                        ...
                        <tbody>
                            {filteredRequests.map(req => (
                                <tr key={req.id} style={{ borderBottom: '1px solid var(--bg-body)' }}>
                                    ...
                                    <td style={{ padding: '15px' }}>{req.device_model}</td>
                                    <td style={{ padding: '15px' }}>{req.condition}</td>
                                    <td style={{ padding: '15px' }}>{req.estimated_price ? `${req.estimated_price} DKK` : '-'}</td>
                                    <td style={{ padding: '15px', fontWeight: 'bold', color: '#10B981' }}>
                                        {req.final_offer_price ? `${req.final_offer_price} DKK` : '-'}
                                    </td>
                                    <td style={{ padding: '15px' }}>
                ...
                <div style={{
                    ...
                    <div style={{ background: 'white', padding: '30px', borderRadius: '16px', width: '500px', maxWidth: '90%' }}>
                        ...
                        <form onSubmit={handleUpdate}>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600' }}>Final Offer Price (DKK)</label>
                                <input
                                    type="number"
                                    value={editingItem.final_offer_price || ''}
                                    placeholder={editingItem.estimated_price}
                                    onChange={(e) => setEditingItem({ ...editingItem, final_offer_price: e.target.value })}
                                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-light)' }}
                                />
                                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '5px' }}>
                                    Estimated: {editingItem.estimated_price} DKK
                                </p>
                            </div>
